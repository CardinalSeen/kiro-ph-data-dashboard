...
import { BudgetLineItems } from './views/BudgetLineItems';
...
function App() {
      ...
      <div className="flex items-center justify-center min-h-screen bg-surface-alt">
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted mt-1">Loading Philippine budget data (6MB)...</p>
          </div>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-surface-alt">
        <div className="bg-surface border border-negative/30 rounded-xl p-6 max-w-md shadow-sm">
          <h2 className="text-base font-bold text-negative mb-2">Data Load Error</h2>
          <pre className="text-xs text-text-secondary bg-surface-alt p-3 rounded-lg overflow-auto max-h-48">{error}</pre>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="flex-1 overflow-hidden flex flex-col">
        ...
        {activeView === 'budget' && <BudgetLineItems query={query} />}
